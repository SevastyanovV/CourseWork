...
сожалению, чтобы выбрать сортировку для запуска, придётся самостоятельно напи-
сать её в теле (с остальными двумя функциями ситуация та же). Для хоть какого-
то удобства вызов сортировки можно скопировать из функции tests(), речь о ко-
...
ется программа Input_maker.c, о ней будет сказано ниже). Подразумевается, что
папки могут иметь только 2 названия: inp16 и inp32 (об этом тоже ниже),
...
названием, как и файл, из которого происходит считывание (файл будет создан в
случае отсутствия), любое другое число - если вывод в файл не нужен.
...
файла выводится "forward order" или "reverse order" соответственно.

ВНИМАНИЕ: в случае большого количества ошибок терминалу будет больно, вывод ин-
дексов ошибок можно отключить в строках 16 и 30!
...
количество файлов каждой размерности, в строке 14 же в массиве dim указать эти
размерности. После этого запустить.
Всё необходимое уже настроено заранее.
